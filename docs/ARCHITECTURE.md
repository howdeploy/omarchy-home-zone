...
атомарно пишет конфиг через `FileView.setText()`. Save переносит в последний
загруженный конфиг только геометрию плиток, поэтому stale draft после reload не
может перезаписать `appIds` или другие settings. Cancel отбрасывает только
