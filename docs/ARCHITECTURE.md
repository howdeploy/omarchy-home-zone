...
перезагружает конфиг на лету. Структура: `display`, `grid`, `card`, `colors`,
`tiles[]`.

Стартовые defaults используются только для отрисовки, пока `FileView` читает
файл. До успешной загрузки запись и открытие Settings заблокированы; временная
ошибка чтения повторяется и не заменяет последний валидный конфиг дефолтами.
...
границы и пересечения, а затем атомарно пишет конфиг через
`FileView.setText()`. Save переносит в последний загруженный конфиг только
