...
`~/.config/omarchy/home-zone.json` — пользовательский конфиг, переживающий обновления плагина. `HomeZone.qml` не открывает этот предсказуемый путь напрямую: короткоживущие `Process` вызывают `/usr/bin/python3 helpers/home_zone_config.py read|write`, а helper сам определяет home по effective UID и принимает операцию, но не путь. Безопасное чтение повторяется раз в две секунды, поэтому внешняя правка применяется без рестарта. Структура: `display`, `grid`, `card`, `colors`, `tiles[]`.

Стартовые defaults используются только для отрисовки, пока helper читает файл. До успешной загрузки запись и открытие Settings заблокированы. Отсутствие конфига в свежей установке разрешает встроенные defaults; ошибка безопасности или исчезновение уже загруженного файла сохраняет последнее валидное состояние и блокирует запись.
...
`display.size` масштабирует всю карточку вместе с плитками, шрифтами и
отступами: `default = 1.0`, `small = 0.8`, `mini = 0.6`. `display.placement`
выравнивает карточку по центру либо по середине выбранного края (`top`,
`right`, `bottom`, `left`) с отступом 48 px. Input-region имеет уже
масштабированный размер и следует за карточкой.
...
размер и расположение Home Zone остаются черновиком до Save: окно позволяет
переносить плитки и менять размер за любую из восьми граней/углов, проверяет
границы и пересечения, а затем передаёт конфиг безопасному helper-процессу для
атомарной записи. Save переносит в последний загруженный конфиг только
геометрию плиток и `display`, поэтому stale draft после reload не может
перезаписать `appIds` или другие settings. Cancel отбрасывает черновик.
Фиксированная сетка `10 × 4` не редактируется; при переносе между заполненными
рядами их содержимое меняется местами, внутри ряда плитки перепаковываются без
пересечений.
...
- Helper descriptor-относительно проходит от `/` до home и `.config/omarchy`, используя `O_NOFOLLOW`, и проверяет владельца и права каждого каталога.
- Конфиг должен быть regular-файлом владельца процесса с одним hard link, без group/other write и размером не более 64 KiB. Legacy-режим `0644` ужесточается до `0600`.
- Запись создаёт новый файл `0600`, синхронизирует его и атомарно публикует через `os.replace` относительно уже проверенного descriptor каталога.
- В репозитории нет install hook, daemon, service, package-manager действий или повышения привилегий.
